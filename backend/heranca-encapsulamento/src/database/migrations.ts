...
export class Migrations extends BaseDatabase{
    ...
    public insertData = async () => {
        try {
            await Migrations.connection(TABLE_USERS)
                .insert(users)
                .then(() => console.log(`${TABLE_USERS} populated!`))
                .catch((error: any) => this.printError(error))
    
            await Migrations.connection(TABLE_PRODUCTS)
                .insert(products)
                .then(() => console.log(`${TABLE_PRODUCTS} populated!`))
                .catch((error: any) => this.printError(error))
    
            await Migrations.connection(TABLE_PURCHASES)
                .insert(purchases)
            ...
            return Migrations.connection.destroy()
        }
    }
